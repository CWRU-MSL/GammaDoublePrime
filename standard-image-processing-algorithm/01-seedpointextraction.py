        ...
        for j in range(0, (original.shape[1] - 1)):
            ...
            if (gnorm > 0):
                ...
                if (bright):
                    ...
                    M_n[ppve[0].astype(int)][ppve[1].astype(int)] = M_n[ppve[0].astype(int)][
                                                                        ppve[1].astype(int)] + gnorm
                if (dark):
                    pnve = p - gp
                    pnve = pnve + offset
                    O_n[ppve[0].astype(int)][ppve[1].astype(int)] = O_n[ppve[0].astype(int)][ppve[1].astype(int)] - 1
...
# plt.show()
#plt.savefig(RESULT_PATH + '01-seedpointextraction.png', dpi=300)
np.save(TEMP_PATH + 'seedpoints.npy', seedpoints_final)
